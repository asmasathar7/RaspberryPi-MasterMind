...
int waitForButton(uint32_t *gpio, int button) {
  ...
  while (1)
  {
    int state = readButton(gpio, button); //calls readButton() to read the state of the button and stores it as state

    fprintf(stderr, "Button state: %d\n", state); //printing the state of the button
    if (state == HIGH)  //if state is HIGH, meaning that the button pressed 
    {
      fprintf(stderr, "Button pressed\n");
      return 1;
      break;
    }
    else  //if state is LOW, meaning button is not pressed 
    {
      struct timespec sleeper, dummy; //declaring two variables of type struct timespec named sleeper and dummy. These structures are used to specify a sleep time in the nanosleep function
      sleeper.tv_sec = 0; // sets the tv_sec field of the sleeper structure to 0, indicating that the sleep time in seconds is 0
      sleeper.tv_nsec = 100000000; // sets the tv_nsec field of the sleeper structure to 100000000 nanoseconds, equivalent to 0.1 seconds
      nanosleep(&sleeper, &dummy); //calls nanosleep function to intorduce a delay of 0.1 second before checking the button state again
      break;
    }
  }
}
...
void initSeq() {
  ...
  theSeq = (int *)malloc(SEQL * sizeof(int)); // allocating memory for theSeq of size SEQL
  if(theSeq == NULL){ //chechking if memory allocation was successful
      printf("Failed to allocate memory \n ");
      exit(EXIT_FAILURE);
  }
  //memory allocation successful 
  for(int i = 0; i < SEQL; i++){  //loop to generate a random number of sequence of size SEQL, which is 3
    theSeq[i] = rand() % SEQL + 1; //assigns a random number between 1 and 3 to each element of the theSeq array
  }
};
...
void showSeq(int *seq) {
  ...
  for (int i = 0; i < SEQL; i++){ //printing the sequence 
    printf("%d ",seq[i]);
  }
...
int countMatches(int *seq1, int *seq2) {
    int exact = 0;  //variable to store exact matches 
    int approx = 0; //variable to store approximate matches
    for (int i = 0; i < SEQL; i++){
      if (seq1[i] == seq2[i]){  // checks if the elements at index i in seq1 and seq2 are equal, indicating an exact match
        exact++; //exact incremented by 1 
        }
      else{ //if elements are not equal of both sequences 
        for (int j = 0; j < SEQL; j++){ //loop to compare the element at index i in seq2 with all elements in seq1 to find approximate match
          if (seq2[i] == seq1[j] && seq1[j] != seq2[j]){ //checks if the element at index i in seq2 is equal to an element at index j in seq1 but not in the same position, indicating an approximate match
            approx++; //aprox incremented by 1 
            break;
            }
          }
        }
...
extern int matches(int* seq1, int* seq2); //external function matches defined in assembly 
...
void showMatches(int /* or int*/ code, /* only for debugging */ int *seq1, int *seq2, /* optional, to control layout */ int lcd_format) {
  /* *  COMPLETE the code here  *  0*/
  int ptr=matches(seq1, seq2); //calls the matches function with seq1 and seq2 as arguments and stores the result in the ptr variable

  printf("%d exact\n%d approximate\n", ptr/10, ptr%10); //prints the exact and approximate matches by dividing ptr by 10 to get the exact matches and using the modulus operator % to get the approximate matches
...
void readSeq(int *seq, int val) {
  /* *  COMPLETE the code here  *  */
  for (int i = SEQL - 1; i >= 0; i--){ //starts a loop that iterates over the array seq in reverse order, starting from the last element
    seq[i] = val % 10; // assigns the last digit of val to the current element of seq
    val = val/10;
  }
}
...
int readNum(int max) {
  /* *  COMPLETE the code here  *  */
  int number;
  printf("Enter a number, maximum:  %d: ", max); //prompts the user to enter a max number
  scanf("%d", &number);
  return number;
}
...
void timer_handler (int signum) { //this fucntion will be called when timer expires
  /* *  COMPLETE the code here  *  */
...
void initITimer(uint64_t timeout){  //This function initializes time-stamps, sets up an interval timer, and installs the timer_handler callback.
  /* *  COMPLETE the code here  *  */
   struct itimerval timer;
  // Set up the timer
  timer.it_value.tv_sec = timeout / 1000000;     //sets the initial timer expiration time in seconds based on the timeout argument
  timer.it_value.tv_usec = timeout % 1000000;    //sets the initial timer expiration time in microseconds based on the timeout argument
  timer.it_interval.tv_sec = timeout / 1000000;  //sets the interval for the timer to repeat in seconds based on the timeout argument.
  timer.it_interval.tv_usec = timeout % 1000000; //sets the interval for the timer to repeat in microseconds based on the timeout argument

  // Install timer_handler as the signal handler for SIGVTALRM
  struct sigaction sa;
  sa.sa_handler = &timer_handler; //sets the signal handler function to timer_handler
  sa.sa_flags = SA_RESTART; //sets the flag to restart system calls if they are interrupted by the signal
  sigfillset(&sa.sa_mask);  //adds all signals to the signal mask, ensuring that no signals are blocked.
  if (sigaction(SIGVTALRM, &sa, NULL) == -1) //installs the timer_handler function as the handler for the SIGVTALRM signal
  {
  ...
  if (setitimer(ITIMER_VIRTUAL, &timer, NULL) == -1) //sets up the interval timer to expire after the specified time interval
  {
...
{ // this is just a suggestion of some variable that you may want to use
  ...
  /* *  COMPLETE the code here  *  */
  //LEDs should be set as output whereas button as input
...
//	mode which sends the commands as 2 x 4-bit values.
  ...
  // -----------------------------------------------------------------------------
  // +++++ main loop
  digitalWrite(gpio, pinLED, LOW); //turning green LED off
  digitalWrite(gpio, pin2LED2, LOW); //turning red LED off
    lcdPosition(lcd, 0, 0);
    lcdPuts(lcd, "Start Game"); //displays start game on the lcd display
    delay(2000); //delay of 2 seconds to keep the display message
    lcdClear(lcd); //clears the lcd display
    sprintf(buf,"Enter Sequence  of %d",SEQL);
    lcdPuts(lcd, buf); //displays this message on the lcd
    delay(2000);


  while (!found) { //while sequence is not found, this loop keeps on iterating
    lcdClear(lcd); //clears the lcd from previous display message
    int turnnum = 0; //variable to count number of turns 
    
    attempts+=1;


   
    //Reads the player's input sequence of colors using button presses.
    while(1){
      turnnum+=1; //turnnum incremented to one so that sequence number 1 is displayed
      sprintf(buf,"Enter Sequence  Number: %d",turnnum);
      lcdPuts(lcd, buf);
      delay(2000);  //delays for 2 seconds
      lcdClear(lcd); //clears the lcd after 5 seconds
      
      //start and end time ramge defined
      //time_t start = time(NULL); 
      time_t tot_time = time(NULL) + 5;

      int buttonpress = 0; //variable to count button presses in order to determine the input

      while(time(NULL) < tot_time){  //while time is in range
        if(waitForButton(gpio,pinButton) == 1){ //wait for button press, if pressed
          buttonpress++; //increment buttonpress
          delay(500); //delay for 0.5 secs
        }
        if(buttonpress >= 3){ //if button press is 3 or greater than 3 we break the loop, since sequence length is of 3 only
          break;
        }
      }
      printf("Button pressed %d times\n", buttonpress); //printing the number of times button was pressed

      digitalWrite(gpio, pin2LED2, HIGH);//making the red led on
      delay(2000); //2 seconds delay
      digitalWrite(gpio, pin2LED2, LOW);//making the red led off after 2000 ms

      blinkN(gpio, pinLED, buttonpress);//blinking the green led as many times the button is pressed

      attSeq[turnnum - 1] = buttonpress; //storing buttonpress (input from user) in attSeq
      ...
      if(turnnum == SEQL){  //if three turnd completed, we print the sequence user entered

        blinkN(gpio, pin2LED2, 2); //blink red led twice to indicate end of an round
        break;
      }
    }
    delay(2000); // 2 seconds of pause before showing exact and approximate matches

   
    int code = matches(theSeq, attSeq);
    //separating exact and approx matches
    int exact = code/10;
    int approx = code%10;
 
    
    //printing exact and approximate matches in terminal
    printf("%d exact\n%d approximate\n", exact, approx);
    ...
    lcdClear(lcd); //clears the lcd display
    blinkN(gpio, pinLED, exact); //green blinks exact number of times showing exact matches
    sprintf(buf, "Exact: %d", exact); //prints exacts matches on lcd
    lcdPosition(lcd, 0, 1);
    lcdPuts(lcd, buf);
    delay(2000); //delay of 2 seconds


    blinkN(gpio, pin2LED2, 1); //blinking red as a separator 
    delay(2000); 


    lcdClear(lcd); //clear lcd display
    blinkN(gpio, pinLED, approx); //blinking green aprroximate number of times
    sprintf(buf, "Approximate: %d", approx); //printing approx matches on lcd
    lcdPosition(lcd, 0, 1);
    lcdPuts(lcd, buf);
    delay(3000);

    if (exact == 3){  //if exact matches equal 3, meaning sequence found  so we break the loop
      found = 1;
      break;
    }

    blinkN(gpio, pin2LED2, 3); //blink red led 3 times to show end of game

    //if not found, we move to the next turn
    delay(500);
    lcdClear(lcd);
    printf("Next Turn\n");
    lcdPuts(lcd,"Next Turn");
    delay(2000);
  ...
  if(found){
    ...
    delay(2000); //delay of 2 seconds
    //while red is on, green blinks thrice indicating success
    digitalWrite(gpio, pin2LED2, HIGH);
    blinkN(gpio, pinLED, 3);
    digitalWrite(gpio, pin2LED2, LOW); 

    //displaying number of attempts on LCD
  ...
  {
    fprintf(stdout, "Sequence is not found\n");
  }
  return 0;
}
